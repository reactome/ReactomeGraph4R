---
title: "R Interface for Reactome Graph Database"
author: Chi-Lam Poon
...
Pathways, reactions, and biological entities in Reactome knowledge are systematically represented as an ordered network of molecular reactions. Graph database technology is an effective tool for modeling highly connected data, hence Reactome database is imported in [Neo4j](https://neo4j.com/) to create one large interconnected graph. Instances are represented as nodes and relationships between nodes as edges.

The [`reactome.graphdb4r`](https://github.com/reactome/reactome.graphdb4r) package is an R interface for retrieving **data with network structure** from [Reactome Graph Database](https://reactome.org/dev/graph-database). There is another R package [`reactome4r`](https://github.com/reactome/reactome4r) for querying **specific bits of information** to the Reactome Database through RESTful APIs in Content Service.

With `reactome.graphdb4r`, you are able to fetch the interconnected data easily in R without the prerequisite of Neo4j's graph query language Cypher. This package is built on the Neo4j driver [`neo4r`](https://github.com/neo4j-rstats/neo4r), thus returned data are mainly same as those called by `neo4r` but with a little modifications, and are in these two formats:
...
Follow this [instruction]() to download and setup Reactome Graph Database.

Then install `reactome.graphdb4r` package via CRAN:
```{r install, eval=FALSE}
install.packages("reactome.graphdb4r")
```

There are two questions needed to be answered for Neo4j server connection when loading the package. You can change the url port if it's not `7474`. And if the Neo4j authentication is required, the username and password are same as the ones to login your local Neo4j database.
...
The function `matchObject` allows you to fetch Reactome objects using:

- `id`: Reactome or non-Reactome identifier 
...
Moreover, you could specify the argument `returnAttribute` for retrieving only a few attributes of the targeted object; `species` for specific species; and `limit` for the number of returned objects. 
Note that this function only returns "row" data.

### Fetch by id

The "id" input can be either non-Reactome or Reactome identifiers. If you use a non-Reactome id, remember that you _must_ specify `databaseName` as well since the default one is "Reactome". For example, to get the Reactome instance associated with the circadian rhythmic gene _PER2_:
...
When retrieving instances belong to one schema class, it's better specify the argument `limit` as well for restricting the number of returned instances. All available schema classes see Reactome [Data Schema](https://reactome.org/content/schema). For instance, to get 5 "EntitySets" in human and then return their display names and stId only:

```{r basic-class}
# Get 5 instance in Class EntitySet
entity.set <- matchObject(schemaClass = "EntitySet", species = "human",
                          returnAttribute = c("displayName", "stId"), limit = 5)
...
The actual Cypher query for this command is `MATCH (n1)-[r:relationship]->(n2) RETURN n1,n2`, therefore `n1` and `n2` dataframes in the returned list have the same number of rows, and every two rows with the same index are connected with the given relationship.
...
`otherReactionLikeEvent` are RLEs connected with Pathway "OAS antiviral response".
...
These Event context can actually be visualized in R using the `exportImage` function from the `reactome4r` package! And it looks the same as that in [Pathway Browser](https://reactome.org/PathwayBrowser/#/R-HSA-8983711&PATH=R-HSA-168256,R-HSA-1280215,R-HSA-913531,R-HSA-1169410). 
To get the pathway diagram of Pathway "OAS antiviral response" (stId: R-HSA-8983711), and highlight the RLE (stId: R-HSA-8983688) that we specified:
...
With the diagram shown above, we can see that the Reaction highlighted in blue is in the middle of somewhere and is immediately connected with two other RLEs. In order to know what these preceding and following Reactions are, we can use function `matchPrecedingAndFollowingEvents` to find RLEs linked via "precedingEvent". The argument `depth` is used to describe the "variable length relationships", the default value is 1 (i.e. immediately connected); or you can set `all.depth = TRUE` for retrieving the whole context. Details see `?matchPrecedingAndFollowingEvents`.
...
Usually we query data in a way like parent to child `(parent) --> (child)`, where we only know information about the parent. But with the Graph Database, we are able to search in a reverse direction that is child to parent `(parent) <-- (child)` with child's information only. This "child-to-parent" relationship is called __Referral__. You could carry out the referral fetching by `matchReferrals` that supports Classes "Event", "PhysicalEntity", "Regulation", "CatalystActivity", "ReferenceEntity", "Interaction", "AbstractModifiedResidue" for now. Depth related arguments could also be specified here. More details sees `?matchReferrals`.
...
```
Note that the dbId of endNode (`endNode.dbId` in `$relationships`) is exactly the dbId we just specified.


### Interactors

Interactions of a PhysicalEntity (PE) could be retrieved by `matchInteractors`. This method first finds the ReferenceEntity matched with the PE, then get the Interactions via "interactor" relationship. For example, to get interactions of "FANCM [nucleoplasm]" with stable id "R-HSA-419535":
...
```{r disease}
# Fetch Reactome instances associated with 'neuropathy' in sheep
matchDiseases(displayName = "neuropathy", species = "sheep", type = "row")
```
...
Here we will show a could of examples to generate an interactive network graph after retrieving the specific Reactome graph data. Let's say we want to visualize the hierarchical data of a ReferenceEntity "UniProt:P33992 MCM5".
...
# load
invisible(suppressPackageStartupMessages(lapply(list.pkg, library, character.only = T)))
```
...
nodes <- graph[["nodes"]]
# unnest the 'properties' column of lists
...
visnet <- visNetwork(vis.nodes, vis.edges, main = "The hierarchy of protein MCM5", width = "100%")
visnet
```
...
We can also take a look at another package [`networkD3`](http://christophergandrud.github.io/networkD3/), which generates network graphs using D3 javascript library.
