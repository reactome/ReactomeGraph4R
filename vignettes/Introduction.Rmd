...
The [`ReactomeGraph4R`](https://github.com/reactome/ReactomeGraph4R) package is an R interface for retrieving **data with network structure** from [Reactome Graph Database](https://reactome.org/dev/graph-database). There is another R package [`ReactomeContentService4R`](https://github.com/reactome/ReactomeContentService4R) for querying **specific bits of information** to the Reactome Database through the RESTful API in the Content Service.
...
devtools::install_github("ReactomeGraph4R")
```
...
```{r export-img}
if (!"ReactomeContentService4R" %in% installed.packages()[ ,"Package"]) {
  install.packages(ReactomeContentService4R)
}
